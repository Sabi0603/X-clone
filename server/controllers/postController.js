...
const deletePost = async (req, res) => {
    try {
        ...
        if (post.user.toString() !== req.user._id.toString()) {
            return res.status(401).json({ message: "You're not authorized to delete this post" });
        }
...
const createComment = async (req, res) => {
    try {
        ...
        const updatedPost = await Post.findById(postId).populate("comments.user", "fullName username profileImg");
